...
class Client(object):
    ...
    def __init__(self, api_key, host=None, device=None):
        ...
        if api_key:
            ...
            raise Exception("Please provide an API key to submit your data")

        # which datadog service host to connect to
        self.datadog_host = find_datadog_host()
        if not self.datadog_host:
            raise Exception("DATADOG_HOST env variable not set")
...
def init(api_key, host=None, device=None):
    ...
    """
    return Client(api_key, host, device)
